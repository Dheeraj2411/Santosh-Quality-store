<template>
  <div class="min-h-screen bg-gray-100 flex">
    ...
    <div v-show="isSidebarOpen" class="fixed inset-0 bg-gray-900/50 z-30 md:hidden" @click="isSidebarOpen = false"></div>

    <!-- Sidebar -->
    <aside :class="isSidebarOpen ? 'translate-x-0' : '-translate-x-full'"
      class="w-64 bg-gray-900 text-white flex flex-col min-h-screen fixed top-0 left-0 z-40 transition-transform duration-300 md:translate-x-0">
      <!-- Logo -->
      <div class="p-6 border-b border-gray-800 flex justify-between items-center">
        <Link href="/admin" class="flex items-center gap-3">
          ...
          <div>
            <div class="font-bold text-sm text-orange-400">Santosh Store</div>
            <div class="text-xs text-gray-400">Admin Panel</div>
          </div>
        </Link>
        <button @click="isSidebarOpen = false" class="md:hidden text-gray-400 hover:text-white p-2">
          ✕
        </button>
      </div>
      ...
      <div class="p-4 border-t border-gray-800">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 bg-orange-500 rounded-full flex items-center justify-center text-sm font-bold">
    ...
    <div class="flex-1 flex flex-col min-h-screen transition-all duration-300 md:ml-64 w-full">
      <!-- Top Bar -->
      <header class="bg-white border-b border-gray-200 px-4 md:px-6 py-4 sticky top-0 z-20">
        <div class="flex items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <button @click="isSidebarOpen = true" class="md:hidden p-2 -ml-2 text-gray-600 hover:text-orange-500 focus:outline-none">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
              </svg>
            </button>
            <div>
              <h1 class="text-lg md:text-xl font-bold text-gray-800 truncate">{{ title }}</h1>
              <p v-if="subtitle" class="text-xs md:text-sm text-gray-500 mt-0.5 truncate">{{ subtitle }}</p>
            </div>
          </div>
          <!-- Flash Messages -->
          <div v-if="$page.props.flash?.success || $page.props.flash?.error" class="hidden sm:flex items-center">
            <div v-if="$page.props.flash.success"
              class="bg-green-100 text-green-700 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 max-w-sm truncate">
              ✅ {{ $page.props.flash.success }}
            </div>
            <div v-if="$page.props.flash.error"
              class="bg-red-100 text-red-700 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 max-w-sm truncate">
        ...
        <div v-if="$page.props.flash?.success || $page.props.flash?.error" class="mt-3 sm:hidden">
          <div v-if="$page.props.flash.success" class="bg-green-100 text-green-700 px-3 py-2 rounded text-xs font-medium flex items-center gap-2">
            ✅ {{ $page.props.flash.success }}
          </div>
          <div v-if="$page.props.flash.error" class="bg-red-100 text-red-700 px-3 py-2 rounded text-xs font-medium flex items-center gap-2">
            ❌ {{ $page.props.flash.error }}
          </div>
        </div>
      </header>
